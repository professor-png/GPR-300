...
namespace ew {
	...
	void createSphere(float radius, int numSegments, MeshData& meshData)
	{
		...
		for (int i = 1; i < numSegments; i++)
		{
			...
			for (int j = 0; j <= numSegments; ++j)
			{
				...
				glm::vec3 tan = glm::vec3(cos(theta) * sin(phi), 0, sin(theta) * sin(phi));
	...
	void createCylinder(float height, float radius, int numSegments, MeshData& meshData)
	{
		...
		for (int i = 0; i <= numSegments; i++)
		{
			...
			glm::vec3 tan = glm::vec3(0, 0, 1);

			meshData.vertices.push_back(Vertex(pos, glm::vec3(0, 1, 0), uv, tan));
		}
		...
		for (int i = 0; i <= numSegments; i++)
		{
			...
			glm::vec3 tan = glm::vec3(-cos(thetaStep * i), 0, sin(thetaStep * i));
