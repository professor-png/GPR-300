...
namespace ew {
	void createPlane(float width, float height, MeshData& meshData) {
		...
		Vertex vertices[4] = {
			...
			{glm::vec3(+halfWidth, 0, -halfHeight), glm::vec3(0,1,0), glm::vec2(0,0)}, //BR
			{glm::vec3(+halfWidth, 0, +halfHeight), glm::vec3(0,1,0), glm::vec2(0,0)}, //TR
	...
	void createSphere(float radius, int numSegments, MeshData& meshData)
	{
		...
		meshData.vertices.push_back({ glm::vec3(0,topY,0),glm::vec3(0,1,0), glm::vec2(0, 1)});
		...
		for (int i = 1; i < numSegments; i++)
		{
			...
			for (int j = 0; j <= numSegments; ++j)
			{
				...
				glm::vec2 uv = glm::vec2(atan(z / x) / (2.0f * glm::pi<float>()));

				meshData.vertices.push_back({ position, normal, uv });
			}
		}

		meshData.vertices.push_back({ glm::vec3(0,bottomY,0), glm::vec3(0,-1,0), glm::vec2(0, -1)});
